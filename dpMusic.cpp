...
{
	mp.setMode(mp.online);
	QString searchText = QInputDialog::getText(this, "搜索♪", "请输入歌曲名：");
	showOnlineMusicsList(searchMusic(searchText.toStdString(), MusicType_wy::songs));
}
void dpMusic::showOnlineMusicsList(MusicsFullInfo mfInfo)
{
	QStandardItemModel* model = new QStandardItemModel;
	std::vector<std::string> musicFilePathList;//歌曲url信息
	std::vector<std::string> musicFileNameList;//歌曲文件名信息
	...
	for (int i = 0; i < mfInfo.musicsInformations.titlesList.size(); i++)
	{
		QList<QStandardItem*> items;
		for (int j = 0; j < 4; j++)
			items << new QStandardItem();
		items[0]->setText(QString::fromStdString(mfInfo.musicsInformations.titlesList[i]));
		titlesList.push_back(mfInfo.musicsInformations.titlesList[i]);
		QString artistsText;
		std::vector<std::string> artistsVector;
		for (auto iter = mfInfo.musicsInformations.artistsList[i].begin(); iter != mfInfo.musicsInformations.artistsList[i].end(); iter++)
		{
			artistsText += QString::fromStdString(*iter) + "、";
			artistsVector.push_back(*iter);
		}
		if (artistsText.size() > 0)
			artistsText.removeLast();
		items[1]->setText(artistsText);
		artistsList.push_back(artistsVector);
		items[2]->setText(QString::fromStdString(mfInfo.musicsInformations.albumTitlesList[i]));
		albumTitlesList.push_back(mfInfo.musicsInformations.albumTitlesList[i]);
		std::string durationText;
		long long duration = mfInfo.musicsInformations.duration[i];
		long long minutes = duration / 1000 / 60;
		long long seconds = duration / 1000 % 60;
		if (minutes < 10)
			durationText += '0';
		durationText += std::to_string(minutes) + ':';
		if (seconds < 10)
			durationText += '0';
		durationText += std::to_string(seconds);
		items[3]->setText(QString::fromStdString(durationText));
		musicFilePathList.push_back(mfInfo.musicFilesInfo.filePath[i]);
		musicFileNameList.push_back("");
		model->appendRow(items);
	}
	ui.musicListView->setModel(model);
	mp.setMusicsInformations(titlesList, artistsList, albumTitlesList);
	mp.setMusicFilesInformations(musicFileNameList, musicFilePathList);
}
...
void dpMusic::listViewDoubleClicked(QModelIndex index)
{
	if (!index.isValid())
		return;
	if (mp.getMode() == mp.online)
	{
		std::string url = mp.getMusicFilesInformations()->filePath[index.row()];
		url.pop_back();
		if (!downloadFile(str2wstr_2UTF8(url), str2wstr_2UTF8(mp.getMusicsInformations().titlesList[index.row()] + ".mp3")))
			return;
		mp.getMusicFilesInformations()->filePath[index.row()] = ".\\";
		mp.getMusicFilesInformations()->fileName[index.row()] = mp.getMusicsInformations().titlesList[index.row()] + ".mp3";
	}
